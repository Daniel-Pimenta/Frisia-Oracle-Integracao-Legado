...
create or replace package body xxfr_ri_pck_int_rcvfisico as
  ...
  begin 
    ...
    */
    ...
    for rch in crh(p_operation_id, p_organization_i) loop
      begin
        select cfi.invoice_id, ph.*
        from
          cll_f189_invoices              cfi,
          cll_f189_invoice_lines         cfil,
        ...
        where 1=1
          ...
          --
          and cfi.invoice_id        = rch.invoice_id
        ;
      exception
        when others then
    ...
    print_log('Inserindo dados para HEADER_INTERFACE_ID:    ' || rcv_headers_interface_s.currval);
