...
/* $Header: CLLPRICB.pls 120.209 2019/09/23 14:05:34 vdzana noship $ */
  --
--g_source                          VARCHAR2(25); -- usada no ADD_ERROR -- 27579747
  ...
  --
  TYPE t_cll_f189_operation_tab IS TABLE OF t_cll_f189_operation_rec INDEX
      BY BINARY_INTEGER;
  ...
  begin
    ...
    WHERE interface_operation_id in (select interface_operation_id from  cll_f189_invoices_interface where source = 'NFe Cooperado');
    ...
    SELECT reci.interface_invoice_id
     ...
     WHERE reci.interface_operation_id    = srce.interface_operation_id
      ...
      AND reci.interface_operation_id          = frtf.interface_operation_id
       ...
       AND reci.interface_invoice_id   = NVL(p_interface_invoice_id, reci.interface_invoice_id);
    --
    l_module_name           CONSTANT VARCHAR2(100) := 'CLL_F189_OPEN_PROCESSES_PUB.CREATE_OPEN_INTERFACE';
  ...
  BEGIN
    ...
    FOR r_valida_invoice_header IN c_valida_invoice_header LOOP
        ...
        ---------------------------
        CLL_F189_OPEN_VALIDATE_PUB.GET_ORGANIZATION_ID (p_organization_code    => r_valida_invoice_header.organization_code
        ...
        BEGIN
          SELECT location_id
            INTO l_location_id
            FROM hr_locations
           WHERE (location_id   = r_valida_invoice_header.location_id OR
                  location_code = r_valida_invoice_header.location_code);
        ...
        ---------------------------
        CLL_F189_OPEN_VALIDATE_PUB.GET_INVOICE_TYPE_ID (p_invoice_type_id     => r_valida_invoice_header.invoice_type_id
        ...
        --
        IF  r_valida_invoice_header.organization_id IS NULL AND
            r_valida_invoice_header.organization_code IS NOT NULL THEN
            --
        ...
        END IF;
        --
        IF  r_valida_invoice_header.location_id IS NULL AND
            r_valida_invoice_header.location_code IS NOT NULL THEN
            --
            UPDATE cll_f189_invoice_iface_tmp
        ...
        END IF;
        --
        IF  r_valida_invoice_header.invoice_type_id IS NULL AND
            r_valida_invoice_header.invoice_type_code IS NOT NULL THEN
            --
            UPDATE cll_f189_invoice_iface_tmp
    ...
    FOR r_invoice_header IN c_invoice_header LOOP
        ...
        IF l_cont_oper_loc > 0 OR l_cont_oper_org > 0 THEN
           ...
           END IF; --IF p_open_source = 'RI' THEN
        ELSE -- Organizacao e Local derivados com sucesso, entao deriva o tipo de nota
            ----------------------------
            ...
            l_error_code := NULL;
            --
            print_log('    Chamando CLL_F189_OPEN_VALIDATE_PUB.GET_INVOICE_TYPE_DATA...');
            CLL_F189_OPEN_VALIDATE_PUB.GET_INVOICE_TYPE_DATA (p_invoice_type_id            => r_invoice_header.invoice_type_id
            ...
            --
            print_log('    Saida:'||l_error);
            print_log('    l_invoice_type_id                 :'||l_invoice_type_id);
            print_log('    r_invoice_header.invoice_type_id  :'||r_invoice_header.invoice_type_id);
            print_log('    r_invoice_header.invoice_type_code:'||r_invoice_header.invoice_type_code);
            ...
            print_log('    l_return_customer_flag:'||l_return_customer_flag);
            ...
            ELSE -- Tipo de nota derivado com sucesso, dando inicio as derivacoes do header
               l_debug := $$PLSQL_LINE; print_log('    Err:'||check_erro);
                ...
                --
                CREATE_OPEN_HEADER (p_open_source                   => 'RI'
                                   ,p_interface_invoice_id          => r_invoice_header.interface_invoice_id
                                   ,p_interface_operation_id        => r_invoice_header.interface_operation_id
                                   ,p_organization_id               => NVL(l_organization_id,r_invoice_header.organization_id)
                ...
                IF g_process_flag IS NOT NULL THEN -- encontrou erro na derivacao do header
                   IF p_open_source = 'RI' THEN
                       SET_PROCESS_FLAG(p_process_flag         => g_process_flag ,p_interface_invoice_id => r_invoice_header.interface_invoice_id);
                ...
                ELSE
                    ...
                    CREATE_OPEN_LINES (p_type                         => 'VALIDATION'
                                      ,p_interface_invoice_id         => r_invoice_header.interface_invoice_id
                                      ,p_interface_operation_id       => r_invoice_header.interface_operation_id
                                      ,p_organization_id              => NVL(l_organization_id,r_invoice_header.organization_id)
                                      ,p_location_id                  => NVL(l_location_id,r_invoice_header.location_id)
                    ...
                    ELSE
                       ...
                       -- 23018594 - Start
                       IF g_freight_inter_operation_id <> r_invoice_header.interface_operation_id OR
                          l_frt_inter_organization_id <> r_invoice_header.organization_id OR
                          g_freight_inter_operation_id IS NULL THEN
                          --
                          ...
                          BEGIN
                            FOR r_freight in c_freight LOOP
                              ...
                              CREATE_OPEN_FREIGHT (p_type                       => 'VALIDATION'
                                                  ,p_interface_operation_id     => r_invoice_header.interface_operation_id
                                                  ,p_location_id                => NVL(l_location_id,r_invoice_header.location_id)
                                                  ,p_operating_unit             => l_operating_unit
                                                  ,p_gl_date                    => r_invoice_header.gl_date
                       ...
                       ELSE
                         ...
                         --
                         CREATE_OPEN_INV_PARENTS (p_type                   => 'VALIDATION'
                                                 ,p_interface_invoice_id   => r_invoice_header.interface_invoice_id
                                                 ,p_interface_operation_id => r_invoice_header.interface_operation_id
                                                 ,p_organization_id        => NVL(l_organization_id,r_invoice_header.organization_id)
                                                 ,p_invoice_line_id_par    => l_invoice_line_id_par -- BUG 19943706
                         ...
                         IF g_process_flag IS NOT NULL THEN -- encontrou erro na derivacao da nota complementar
                           ...
                           ELSE
                              ...
                              ELSE
                                --
                                CREATE_OUTBOUND_INVOICES (p_type                   => 'VALIDATION'
                                                         ,p_insert                 => 'N' -- 22346186
                                                         ,p_interface_invoice_id   => r_invoice_header.interface_invoice_id
                                                         ,p_interface_operation_id => r_invoice_header.interface_operation_id
                                                         ,p_organization_id        => NVL(l_organization_id,r_invoice_header.organization_id)
                                ...
                                ELSE
                                  --
                                  CREATE_PRIOR_BILLINGS (p_type                   => 'VALIDATION'
                                                        ,p_interface_invoice_id   => r_invoice_header.interface_invoice_id
                                                        ,p_interface_operation_id => r_invoice_header.interface_operation_id
                                                        ,p_organization_id        => NVL(l_organization_id,r_invoice_header.organization_id)
                                                        ,p_invoice_id             => NULL
                                  ...
                                  ELSE
                                     --
                                     CREATE_PAYMENT_METHODS (p_type                   => 'VALIDATION'
                                                            ,p_interface_invoice_id   => r_invoice_header.interface_invoice_id
                                                            ,p_interface_operation_id => r_invoice_header.interface_operation_id
                                                            ,p_organization_id        => NVL(l_organization_id,r_invoice_header.organization_id)
                                                            ,p_invoice_id             => NULL
                                     ...
                                     ELSE
                                                                          ...
                                                                          --
                                        CREATE_REFERENCED_DOCUMENTS (p_type                   => 'VALIDATION'
                                                                    ,p_interface_invoice_id   => r_invoice_header.interface_invoice_id
                                                                    ,p_interface_operation_id => r_invoice_header.interface_operation_id
                                                                    ,p_organization_id        => NVL(l_organization_id,r_invoice_header.organization_id)
                                                                    ,p_invoice_id             => NULL
        ...
        --print_log('Err-2:'||check_erro); l_debug := $$PLSQL_LINE; --192;
        --
        BEGIN
            ...
            l_debug := $$PLSQL_LINE; print_log('    Err:'||check_erro);  --193;
            --
            print_log('l_qtd_lines_tmp:'||l_qtd_lines_tmp);

            IF  NVL(l_qtd_lines_tmp, 0) > 0 THEN
              print_log('  Chamando XXFR_F189_CHECK_HOLDS_PKG.FUNC_CHECK_HOLDS...');
              l_return_pkg := XXFR_F189_CHECK_HOLDS_PKG.FUNC_CHECK_HOLDS (
                p_organization_id      => NVL(l_organization_id,r_invoice_header.organization_id)
                ,p_location_id          => NVL(l_location_id,r_invoice_header.location_id)
            ...
            ELSE
                ...
                IF g_interface_operation_id    <> r_invoice_header.interface_operation_id OR -- Bug 18043475
                   ...
                   BEGIN
                     ...
                     WHERE ril.interface_invoice_id =  r_invoice_header.interface_invoice_id
                       AND (ril.cfo_code = rfo.cfo_code OR
                            ril.cfo_id = rfo.cfo_id)
                       AND ROWNUM = 1;
                   EXCEPTION
                     WHEN OTHERS THEN
                        l_debit_free       := NULL;
...
-- 28806961_27831745 - End
                      IF l_debit_free = 'Y'
                      OR l_tpa_control_type IS NOT NULL THEN
                         l_status := 'INCOMPLETE';
                      ELSE
                         l_status := 'COMPLETE';
                      END IF;
                   ...
                   CLL_F189_ENTRY_OPERATIONS_PUB.CREATE_ENTRY_OPERATIONS(p_organization_id        => NVL(l_organization_id,r_invoice_header.organization_id)
                                                                        ,p_location_id            => NVL(l_location_id,r_invoice_header.location_id)
                   ...
                   BEGIN
                     FOR r1_freight IN c1_freight LOOP
                       v_source_state_id := CLL_F189_OPEN_VALIDATE_PUB.GET_STATES (p_state_id   => r1_freight.source_state_id
                                                                                  ,p_state_code => r1_freight.source_state_code);
                       v_destination_state_id := CLL_F189_OPEN_VALIDATE_PUB.GET_STATES (p_state_id   => r1_freight.destination_state_id
                                                                                       ,p_state_code => r1_freight.destination_state_code);
                       CREATE_OPEN_FREIGHT (p_type                       => 'INSERT'
                                           ,p_interface_operation_id     => r_invoice_header.interface_operation_id
                                           ,p_location_id                => NVL(l_location_id,r_invoice_header.location_id)
                                           ,p_operating_unit             => l_operating_unit
                                           ,p_gl_date                    => r_invoice_header.gl_date
                ...
                END IF;
                -- 23091360 - End
                CLL_F189_INVOICES_PUB.CREATE_INVOICES (p_invoices_id                    => g_cll_f189_invoices_s
                                                      ,p_entity_id                      => NVL(r_invoice_header.entity_id,l_entity_id_out)
                                                      ,p_invoice_num                    => r_invoice_header.invoice_num
                                                      ,p_series                         => r_invoice_header.series
                                                      ,p_user_id                        => l_user_id
                ...
                --
                IF p_open_source = 'RI' THEN
                    SET_PROCESS_FLAG(p_process_flag         => 6
                                    ,p_interface_invoice_id => r_invoice_header.interface_invoice_id
                                    );
                END IF; --IF p_open_source = 'RI' THEN
...
-- 28806961_27831745 - End
                ...
                CLL_F189_ENTRY_OPERATIONS_PUB.SET_STATUS_ENTRY_OPERATIONS (p_status          => l_rec_status
                                                                          ,p_operation_id    => g_cll_f189_entry_operations_s
                                                                          ,p_organization_id => r_invoice_header.organization_id
                                                                          );
    ...
    FOR g_index IN NVL(r_cll_f189_operation.first, 0)..NVL(r_cll_f189_operation.last, 0) LOOP
       ...
       -- 27579747 - End
           ...
           APPROVE_INTERFACE( p_organization_id => r_cll_f189_operation(g_index).organization_id
  ...
  PROCEDURE CREATE_OPEN_HEADER (p_open_source                   IN VARCHAR2
    ...
    BEGIN
      ...
      END IF;
        ...
        -------------------------------------------
        --
        l_inventory_organization_id := CLL_F189_OPEN_VALIDATE_PUB.GET_INVENTORY_ORGANIZATION (p_organization_id => p_organization_id
...
-- 27579747 - End
          ...
          l_debug := $$PLSQL_LINE; print_log('    Err:'||check_erro);
            ...
            IF (l_qtd_invoices > 0 ) OR (l_qtd_invoices_tmp > 1 ) THEN
                ADD_ERROR(p_invoice_id             => p_interface_invoice_id
                         ,p_interface_operation_id => p_interface_operation_id
                         ,p_organization_id        => p_organization_id
                         ,p_error_code             => 'DUPLICATED INVOICE 37'
                         ,p_invoice_line_id        => 0
                         ,p_table_associated       => 1 -- CLL_F189_INVOICES_INTERFACE
                         ,p_invalid_value          => p_invoice_num
                         );
        ...
        IF NVL(p_qtd_lines_tmp,0) = 0 THEN
            IF NVL(g_generate_line_compl,'N') = 'N'
            OR (NVL(p_cost_adjust_flag ,'N') = 'Y') THEN -- 22080756
                ADD_ERROR(p_invoice_id             => p_interface_invoice_id
                         ,p_interface_operation_id => p_interface_operation_id
                         ,p_organization_id        => p_organization_id
        ...
        ------------------------------------------
        --
        CLL_F189_FISCAL_DOC_TYPE_PKG.GET_FISCAL_DOC_TYPE_SETUP(p_fiscal_document_type_code   => UPPER(p_fiscal_document_model)
                                                              -- out
...
-- 27579747 - End
        ...
        IF p_invoice_parent_id IS NOT NULL THEN
            IF p_parent_flag = 'Y' AND p_cost_adjust_flag = 'N' AND p_price_adjust_flag = 'N' AND p_tax_adjust_flag = 'N' AND p_fixed_assets_flag NOT IN ('S', 'O') THEN
                -- Verify if invoice choosen is on the table cll_f189_invoices
                l_invoice_parent_id := CLL_F189_OPEN_VALIDATE_PUB.GET_INVOICE_PARENTS (p_entity_id         => p_entity_id         -- Bug 18468814
...
-- 27579747 - End
  ...
  BEGIN
    ...
    --
    l_count := 0;
    -- Iniciando validacoes das Lines
    --
    -- ER 9028781 - Start
     BEGIN
        SELECT UPPER(ri.icms_type)
              ,simplified_br_tax_flag
        INTO l_icms_type
            ,l_simplified_br_tax_flag
    ...
    FOR r_invoice_lines IN c_invoice_lines LOOP
     ...
     -- Bug 26908937, 26997066, 25961981 - Fim
        ...
        IF p_type = 'VALIDATION' THEN
            ...
            IF l_iss_flag = 'Y' AND
               NVL(l_simplified_br_tax_flag,'N') <> 'Y' THEN

            l_city_id := CLL_F189_OPEN_VALIDATE_PUB.GET_CITIES (p_iss_city_code => r_invoice_lines.iss_city_code
            ...
            ELSE -- IF r_invoice_lines.line_location_id IS NOT NULL THEN
                IF r_invoice_lines.purchase_order_num IS NOT NULL OR r_invoice_lines.line_num IS NOT NULL OR r_invoice_lines.shipment_num IS NOT NULL THEN
                    CLL_F189_OPEN_VALIDATE_PUB.GET_PURCHASE_ORDERS(p_line_location_id      => NULL
            ...
            IF r_invoice_lines.requisition_line_id IS NOT NULL AND r_invoice_lines.shipment_line_id IS NOT NULL THEN
                --
                l_item_id_rsl := CLL_F189_OPEN_VALIDATE_PUB.GET_REQUISITION_RCV(p_organization_id     => p_organization_id
            ...
            IF r_invoice_lines.requisition_line_id IS NULL AND
               l_requisition_type = 'OE' THEN
                ADD_ERROR(p_invoice_id             => p_interface_invoice_id
                         ,p_interface_operation_id => p_interface_operation_id
                         ,p_organization_id        => p_organization_id
            ...
            --
            IF r_invoice_lines.rma_interface_id IS NOT NULL AND
              (p_qtde_nf_compl = 0 AND p_parent_flag = 'N') THEN -- Bug 16600918
                --
                l_item_id_rma := CLL_F189_OPEN_VALIDATE_PUB.GET_RMA_RECEIPTS(p_entity_id        => p_entity_id
            ...
            IF r_invoice_lines.db_code_combination_id IS NOT NULL THEN
                --
                l_code_combination_id := CLL_F189_OPEN_VALIDATE_PUB.GET_GL_CODE_COMBINATIONS(p_chart_of_accounts_id   => p_chart_of_accounts_id
            ...
            -- 27579747 - Start
             IF g_source = 'CLL_F369 EFD LOADER SHIPPER' THEN
              BEGIN
                ...
                WHERE cfo.cfo_id   = NVL(r_invoice_lines.cfo_id,cfo.cfo_id)
             ...
             ELSE
            -- 27579747 - End
                l_cfo_id := CLL_F189_VALID_RULES_PKG.GET_FISCAL_OPERATIONS_VRULES(p_cfo_id                => r_invoice_lines.cfo_id
            ...
            --        CFOP        --
            ------------------------
            l_cfo_id := CLL_F189_OPEN_VALIDATE_PUB.GET_CFO(p_cfo_id    => NVL(r_invoice_lines.cfo_id,l_cfo_id)
            ...
            ------------------------
            --
            l_utilization_cfo := CLL_F189_OPEN_VALIDATE_PUB.GET_CFO_UTILIZATIONS (p_cfo_id           => NVL(r_invoice_lines.cfo_id,l_cfo_id)
...
-- 29055483 - End
  ...
  BEGIN
    ...
    FOR r_valida_freight IN c_valida_freight LOOP
        ...
        -- Validando Organizacao --
        ---------------------------
        CLL_F189_OPEN_VALIDATE_PUB.GET_ORGANIZATION_ID (p_organization_code    => r_valida_freight.organization_code
...
-- 27579747 - End
 ...
 PROCEDURE CREATE_OPEN_TPA_RETURNS ( p_type                      IN VARCHAR2
  ...
  CURSOR c_invoice_lines_tpa
   ...
   SELECT tmp.tpa_remit_control_id
        , tmp.new_subinventory_code
        , tmp.new_locator_id
        , tmp.new_locator_code
  ...
  BEGIN
    ...
    IF p_type = 'VALIDATE' then
      ...
      --
      --O CFOP determina se a nota ?u n?de poder de terceiros: 'Y' Caso ela seja fazer todas as valida?s abaixo
      --                                                           'N' Seguir em diante sem as valida?s TPA
      l_tpa_control_flag := CLL_F189_OPEN_VALIDATE_PUB.GET_CFO_THIRD_PARTY (p_cfo_id     => l_cfo_id
                                                                           ,p_cfo_code   => NULL);
      --
      BEGIN
       SELECT count(1)
    ...
    /*-- -------------------
      ...
      -- 30211420 - End
      --
      -- Caso exista dados na interface TPA e o CFOP n?seja compat?l gravar erro na open
      --
      IF (l_tpa_control_flag = 'N' and l_count_tpa_return > 0) then
         -- --------------------
         -- TPA INV RETURN CFOP
         -- --------------------
            ADD_ERROR(p_invoice_id             => p_interface_invoice_id --r_tpa_tmp.interface_invoice_id
  ...
  -- IF (l_tpa_control_flag = 'Y' and l_count_tpa_return > 0) and p_return_code = 'S' then -- 30211420
     IF l_tpa_control_flag = 'Y' THEN                                                     -- 30211420
            ...
            p_return_message := 'INV LOCATION ID'; --'INV LOCATION ID';
              */
              /*  ADD_ERROR(p_invoice_id             => p_interface_invoice_id
                         ,p_interface_operation_id => l_interface_operation_id
                         ,p_organization_id        => p_organization_id
                         ,p_error_code             => 'INVALID LOCATION'
                         ,p_invoice_line_id        => p_interface_invoice_line_id
                         ,p_table_associated       => 5 -- CLL_F513_TPA_RETURNS_IFACE
                         ,p_invalid_value          => NULL
                         );
                p_return_code    := 'E';
                p_return_message := 'INVALID LOCATION';
              --
              -- Bug 29449519 - END
              --
            end if;
          END IF;
       END IF; --IF (l_tpa_control_flag = 'Y' and l_count_tpa_return > 0) and p_return_code = 'S' then
       --
	   -- ENR 30120364 - Start
	   --
       BEGIN
       ...
       --
         ...
         ELSE
            ...
            -- TPA RETURN REQUIRED
            -- -------------------
            IF l_tpa_control_flag = 'Y' AND l_count_tpa_return = 0
            AND l_tpa_mat_off = 0 THEN
    ...
    FOR r_payment_methods IN c_payment_methods(p_interface_invoice => p_interface_invoice_id) LOOP
        ...
        ELSIF p_type = 'INSERT' THEN
            ...
            BEGIN
                ...
                CLL_F189_PAYMENT_METHODS_PUB.CREATE_PAYMENT_METHODS(p_payment_method_id        => l_payment_method_id
                                                                   ,p_invoice_id               => p_invoice_id
  ...
  BEGIN
    ...
    FOR r_referenced_documents IN c_referenced_documents(p_interface_invoice => p_interface_invoice_id) LOOP
        ...
        ELSIF p_type = 'INSERT' THEN
            ...
            BEGIN
                --
                CLL_F189_REF_DOCS_PUB.CREATE_REFERENCED_DOCUMENTS(p_referenced_documents_id   => l_referenced_documents_id
                                                                 ,p_invoice_id                => p_invoice_id
    ...
    print_log('  APPROVE_INTERFACE');
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ...
                raise_application_error (-20001,SQLERRM|| '- SELECT org_organization_definitions - '
                                                       || ' organization_id:'
                                                       || p_organization_id
                                                       || ' operation_unit:'
                                                       || l_operating_unit);
...
-- Iniciar o tratamento dos recebimentos originados pelo EFD_Loader
           ...
           cll_f189_loader_adjust_pkg.adjust_process(p_organization_id   => p_organization_id
                                                    ,p_operation_id      => p_operation_id
                                                    ,p_source            => p_source
...
-- Acionar a rotina de calculo dos impostos
              ...
              cll_f189_loader_adjust_pkg.calculate_taxes(p_organization_id => p_organization_id
                                                        ,p_operation_id    => p_operation_id
                                                        ,p_tax             => 'OTHERS'
                                                        ,p_return_status   => l_return_status );
              IF l_return_status = '1' THEN
...
-- 28806961_27831745 - End
        ...
        --
        CLL_F189_ICMS_TAX_PKG.get_icms_st_advanced (p_organization_id => p_organization_id
                                                   ,p_location_id     => p_location_id
                                                   ,p_operation_id    => p_operation_id
                                                   );
        --
        CLL_F189_ICMS_TAX_PKG.get_icms_st (p_organization_id   => p_organization_id
                                          ,p_location_id       => p_location_id
                                          ,p_operation_id      => p_operation_id
        ...
        --
        CLL_F189_FREIGHT_DIFFERENT_PKG.PROC_FREIGHT_DIFFERENTIAL (p_operation_id
                                                                 ,p_location_id
                                                                 ,p_organization_id
                                                                 );
        --
        ...
        BEGIN
            SELECT COUNT(1)
             ...
             WHERE cfi.organization_id            = p_organization_id
        ...
        IF l_outbound_qty > 0 THEN
            CLL_F189_OUTBOUND_INVOICES_PKG.CALC_RATIO_FRT (p_operation_id
                                                          ,p_organization_id
                                                          );
        END IF;
        --
        CLL_F189_AVERAGE_COST_PKG.PROC_AVERAGE_COST (p_operation_id
                                                    ,p_organization_id
                                                    ,p_location_id
                                                    );
        ...
        --
        CLL_F189_CALC_IDX_DOLLAR_PKG.PROC_CALC_IDX_DOLLAR (p_operation_id
                                                          ,p_location_id
                                                          ,p_organization_id
                                                          ,'N'
                                                          ,NULL               -- 23275142
        ...
        --
        UPDATE cll_f189_entry_operations
           SET translation_factor    = l_trans_factor
        ...
        BEGIN
            ...
            IF l_contab_flag = 'I'THEN
                CLL_F189_TAX_ONLY_PKG.PROC_TAX_ONLY (p_operation_id
                                                    ,l_operating_unit
                                                    ,p_organization_id
                                                    ,l_gl_date
                                                    ,p_receive_date
            ...
            ELSE
                CLL_F189_POST_TO_GL_PKG.PROC_POST_TO_GL (p_operation_id
                                                        ,l_operating_unit
                                                        ,p_organization_id
                                                        ,l_gl_date
                                                        ,p_receive_date
        ...
        IF l_return_flag IS NOT NULL THEN
            ...
            BEGIN
                ...
                CLL_F189_INTERFACE_PKG.AP (p_operation_id,
                                           p_organization_id,
                                           l_operating_unit,
                                           p_created_by);
                --
                ...
                BEGIN
                    --
                    IF l_contab_flag = 'N' OR
                       l_contab_flag = 'P' THEN
                         --
                         l_posted_flag := 'N';
                         --
                    ELSE
...
END IF; --  l_return_status = '0' -- 28806961_27831745
  ...
  PROCEDURE BASE_ACCUMULATE_IRRF (p_accumulate_type            IN cll_f189_tax_sites.cumulative_threshold_type%TYPE DEFAULT NULL
    ...
    l_module_name CONSTANT VARCHAR2 (100) := 'CLL_F189_OPEN_PROCESSES_PUB.BASE_ACCUMULATE_IRRF' ;
    --
    CURSOR irrf1 IS
  ...
  PROCEDURE UPDATE_IRRF_WITHHOLD_INVOICE (p_accumulate_type   IN cll_f189_tax_sites.cumulative_threshold_type%TYPE DEFAULT NULL
    ...
    l_module_name  CONSTANT VARCHAR2 (100):= 'CLL_F189_OPEN_PROCESSES_PUB.UPDATE_IRRF_WITHHOLD_INVOICE' ;
    --
    CURSOR irrf1 IS
  ...
  --
    l_module_name       CONSTANT VARCHAR2(100) := 'CLL_F189_OPEN_PROCESSES_PUB.SET_PROCESS_FLAG';
  --
  BEGIN
    print_log('    SET_PROCESS_FLAG -->'||p_process_flag);
    print_log('    Debug:'||l_debug);
    BEGIN
      UPDATE cll_f189_invoices_interface
  ...
  PROCEDURE add_error ( p_invoice_id             IN NUMBER
    ...
    --
    l_module_name CONSTANT VARCHAR2 (100) := 'CLL_F189_OPEN_PROCESSES_PUB.ADD_ERROR' ;
    l_error_message        VARCHAR2 (240);
    --
  BEGIN
    print_log('    ADD_ERROR -> '||l_debug);
    BEGIN
        SELECT description
          INTO l_error_message
          FROM fnd_lookup_values_vl
         WHERE lookup_type = 'CLL_F189_INTERFACE_HOLD_REASON'
           AND lookup_code = p_error_code ;
        print_log('    Erro Code:'||p_error_code);
    EXCEPTION
        WHEN OTHERS THEN
    ...
    --
    BEGIN
        INSERT INTO cll_f189_interface_errors ( interface_operation_id
                                              , interface_invoice_id
                                              , interface_invoice_line_id
                                              , error_code
                                              , error_message
    ...
    ELSE
        ...
        BEGIN
            ...
            --
            print_log('    Chamando CLL_F189_HOLDS_CUSTOM_PKG.FUNC_HOLDS_CUSTOM...');
            l_holds := CLL_F189_HOLDS_CUSTOM_PKG.FUNC_HOLDS_CUSTOM (p_interface_operation_id
                                                                   ,p_organization_id
                                                                   );
            --
            BEGIN
