    ...
    FOR r_invoice_header IN c_invoice_header LOOP
        ...
        BEGIN
            ...
            IF  NVL(l_qtd_lines_tmp,0) > 0 THEN
                --
                l_return_pkg := CLL_F189_CHECK_HOLDS_PKG.FUNC_CHECK_HOLDS (p_organization_id      => NVL(l_organization_id,r_invoice_header.organization_id)
                                                                          ,p_location_id          => NVL(l_location_id,r_invoice_header.location_id)
...
--            END IF; -- ER 14124731 -- 28806961_27831745
    ...
    IF p_delete_line = 'Y' THEN
        DELETE cll_f189_interface_errors
         WHERE interface_operation_id IN (SELECT DISTINCT interface_operation_id
        ...
        DELETE cll_f189_invoice_line_par_int
         WHERE interface_parent_id IN (SELECT interface_parent_id
  ...
  PROCEDURE CREATE_OPEN_HEADER (p_open_source                   IN VARCHAR2
   ...
   BEGIN
     print_log('  CREATE_OPEN_HEADER');
        ...
        l_error_code := NULL;
        --
        CLL_F189_OPEN_VALIDATE_PUB.GET_PARAMETERS (p_organization_id            => p_organization_id
                                                   -- out
       ...
       --
        ...
        ELSIF p_invoice_amount <= 0 AND NVL(p_utilities_flag,'N') = 'N'
          AND NVL(p_tax_adjust_flag,'N') <> 'Y' THEN -- 22462893
            ADD_ERROR(p_invoice_id             => p_interface_invoice_id
                     ,p_interface_operation_id => p_interface_operation_id
                     ,p_organization_id        => p_organization_id
        ...
        ELSIF p_gross_total_amount <= 0 AND NVL(p_utilities_flag,'N') = 'N'
          AND NVL(p_tax_adjust_flag,'N') <> 'Y' THEN -- 22462893
            ADD_ERROR(p_invoice_id             => p_interface_invoice_id
                     ,p_interface_operation_id => p_interface_operation_id
                     ,p_organization_id        => p_organization_id
...
END IF; --  l_return_status = '0' -- 28806961_27831745
  ...
  EXCEPTION
        WHEN OTHERS THEN
            ...
            print_log( ' ');
            --
            raise_application_error (-20002,l_module_name|| ' - ERROR l_debug number: { '
                                                         || TO_CHAR (l_debug)
                                                         ||' } - '
                                                         || SQLERRM);
  ...
  BEGIN
    ...
    g_process_flag := 3;
    --
    IF p_error_code = 'NONE ORGANIZATION' OR
       p_error_code = 'NONE LOCATION' THEN
       NULL;
    ELSE
        DECLARE
            l_holds   NUMBER;
