create or replace PACKAGE BODY XXFR_RI_PCK_INT_NFDEVOLUCAO AS
  ...
  begin   
    print_log('');
    print_log('XXFR_RI_PCK_INT_NFDEVOLUCAO.PROCESSA_AUTO_INVOICE - '||to_char(sysdate,'HH24:MI:SS'));
  ...
  begin
    ...
    if (ok and isCommit) then
      ...
      --
      select a.cust_trx_type_id, c.name, a.trx_number, a.serie_number
  ...
  begin
    g_cd_chave_acesso := p_cd_chave_acesso;
    x_retorno := l_retorno;
  end;
  ...
  begin
    print_log('XXFR_RI_PCK_INT_NFDEVOLUCAO.GERA_INTERFACE - '||to_char(sysdate,'HH24:MI:SS'));
    if (ok) then
    ...
    if (isOnlyAR = false) then
      ...
      */
      ...
      if (ok and e = 0 and isCommit) then
        ...
        print_log('Interface AR   :'||w_ar_interface_flag);
