...
declare 
  ...
  l_interf_header_attribute15   varchar2(50);
...
begin
  if (:new.electronic_inv_status = 2) then
    ...
    begin
      ...
      into l_attrib_category, l_attribute15, l_interf_header_attribute15, l_tipo_operacao
      from 
    ...
    -- INTEGRAÇÃO LEGADO SIF
    if (l_interf_header_attribute15 <> 'Y') then
      l_boolean := FND_REQUEST.set_mode(TRUE);
      l_request_id := FND_REQUEST.submit_request(
        application => 'XXFR',
        program     => 'XXFR_AR_PUBLICA_NFE_LEGADO',
        description => null,
        start_time  => sysdate,
        sub_request => false,
        argument1   => l_customer_trx_id
