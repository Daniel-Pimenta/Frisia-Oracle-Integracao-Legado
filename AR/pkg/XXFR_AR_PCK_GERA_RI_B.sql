create or replace PACKAGE BODY XXFR_AR_PCK_GERA_RI AS
  ...
  begin
    ...
    exception when others then 
      print_log('Ambiente não iniciado:'||sqlerrm);
      ok:=false;
      return;
    end;
    ...
    begin
      print_log('');
      print_log('Informações da NF de Entrada (AR)');
  ...
  begin
    ...
    for r1 in c1 loop
      ...
      begin
        ...
        ;
      exception when others then
        print_log('** Erro ao buscar a codição de pagamento padrão [NAO SE APLICA]');
        x_retorno := 'E';
        ok:=false;
        return;
      end;
  ...
  begin
    ...
    exception 
      when no_data_found then
        ok:=false;
        x_retorno := 'Transacao do RI não encontrada. Verificar Plano de Coleta [Org Inventario][Tipo de Transação] ['||p_warehouse_id||']['||w_cust_trx_type_name||']';
        print_log(x_retorno);
        return;
      when others then
        ok:=false;
