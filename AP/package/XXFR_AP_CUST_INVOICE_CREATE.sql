...
BEGIN
  ...
  dbms_output.put_line('INICIO DO PROCESSAMENTO - '||to_char(sysdate,'DD/MM/YYYY HH24:MI:SS')||' - CRIAÇÃO NFF - PO:'||p_segment1);
  dbms_output.put_line('********************************************************************');
  --
  ...
  if (l_external_bank_account_id is null) then
    dbms_output.put_line('O Fornecedor ['||l_vendor_name||'] não tem Conta Bancaria cadastrada !');
    ok:=false;
  end if;
  ...
  dbms_output.put_line('********************************************************************');
  dbms_output.put_line('FIM DO PROCESSAMENTO - '||to_char(sysdate,'DD/MM/YYYY HH24:MI:SS')||' - CRIAÇÃO NFF - PO:'||p_segment1);
  dbms_output.put_line('********************************************************************');
EXCEPTION
